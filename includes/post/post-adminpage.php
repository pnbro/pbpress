...
function _pb_post_rewrite_handler_for_adminpage($rewrite_path_){
	...
	$other_post_ = pb_hook_apply_filters('pb_adminpage_manage_post_rewrite_handler', null, $sub_action_, $rewrite_path_);

	if(pb_is_error($other_post_)){
		return $other_post_;
	}

	if(strlen($other_post_)){
		return $other_post_;
	}
...
function _pb_post_installed_tables(){
	$check_ = pb_authority_task_by_slug(PB_AUTHORITY_SLUG_ADMINISTRATOR, "manage_post");
...
function _pb_post_ajax_edit(){
	...
	echo json_encode(array(
		'success' => true,
		'post_id' => $post_id_,
		'redirect_url' => pb_admin_url("manage-post/edit/".$post_id_),
	));
	pb_end();
...
function _pb_post_ajax_delete(){
	...
	echo json_encode(array(
		'success' => true,
		'redirect_url' => pb_admin_url("manage-post"),
	));
	pb_end();
...
function _pb_post_ajax_register_front_post(){
	if(!pb_user_has_authority_task(pb_current_user_id(), "manage_post")){
		echo json_encode(array(
			"success" => false,
			"error_title" => "권한없음",
...
pb_add_ajax('register-front-post', "_pb_post_ajax_register_front_post");

function _pb_post_ajax_unregister_front_post(){
	if(!pb_user_has_authority_task(pb_current_user_id(), "manage_post")){
		echo json_encode(array(
			"success" => false,
			"error_title" => "권한없음",
...
pb_add_ajax('unregister-front-post', "_pb_post_ajax_unregister_front_post");
