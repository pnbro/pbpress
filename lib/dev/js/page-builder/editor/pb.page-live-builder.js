...
jQuery(function($){
	...
	pb_page_live_builder.prototype._open_edit_panel = function(path_){
		...
		PB.post('page-builder-load-edit-form', {
			...
			// 폼 HTML 주입
			// ★ serialize_object() 는 내부에서 serializeArray() 를 쓰므로 반드시 진짜 <form> 이어야 한다.
			//    (<div> 래퍼면 폼 값이 통째로 빈 값이 된다)
			//    엔터키 등으로 실제 전송되지 않도록 onsubmit="return false".
			self_._side_panel_body.html('<form class="le-edit-form-wrap" onsubmit="return false;">' + j_.form_html + '</form>');
	...
	pb_page_live_builder.prototype._apply_edit_changes = function(){
		...
		// ── 폼 데이터 수집 ──
		// ★ 파일/이미지 업로드 위젯(pb_file_input / pb_image_input)의 값은 DOM value 에 반영되지 않고
		//    'pb-serialize-object' 데이터필터를 통해서만 노출된다. 반드시 serialize_object() 를 쓸 것.
		//    (직접 .val() 로 긁으면 업로드 결과가 통째로 유실된다)
		if(!this._edit_form || !this._edit_form.length) return;

		// 위지윅 등 submit 시점 동기화 훅 실행 (onsubmit="return false" 이므로 실제 전송은 없음)
		this._edit_form.triggerHandler('submit');

		var form_data_ = this._edit_form.serialize_object();
		var content_ = '';

		// content 필드는 properties 가 아니라 elementcontent 로 분리
		if(form_data_.hasOwnProperty('content')){
			content_ = form_data_['content'];
			delete form_data_['content'];
		}

		// serialize_object 는 미체크 체크박스를 null 로 준다 → 기존 동작('')과 맞춘다
		this._edit_form.find('input[type="checkbox"][name]').each(function(){
			var cname_ = $(this).attr('name');
			if(form_data_[cname_] === null || form_data_[cname_] === undefined) form_data_[cname_] = '';
		});
		...
		// ★ 모달 빌더(pb.page-builder.js)와 저장 포맷을 일치시킨다.
		//    업로드 위젯이 돌려주는 객체/배열 값은 JSON 문자열로 보관해야
		//    기존에 저장된 데이터·테마 코드와 동일한 형태가 된다.
		if(!el_data_.properties) el_data_.properties = {};
		for(var key_ in form_data_){
			if(form_data_.hasOwnProperty(key_)){
				var value_ = form_data_[key_];
				if(value_ !== null && typeof value_ === 'object'){
					value_ = JSON.stringify(value_);
				}
				el_data_.properties[key_] = value_;
			}
		}
