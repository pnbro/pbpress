...
jQuery(function($){
	...
	pb_page_live_builder.prototype._show_select_overlay = function(path_){
		...
		var child_dropdown_ = '';
		if(children_.length > 0){
			child_dropdown_ = '<div class="le-child-dropdown" style="display:none;">'
			...
			for(var ci_ = 0; ci_ < children_.length; ci_++){
				...
				child_dropdown_ += '<div class="le-child-item" data-child-path="'+cpath_+'">'
		...
		// ── 자식 목록 항목 클릭 이벤트 ──
		this._overlay_layer.find('.le-select-overlay .le-child-item').on('click', function(e_){
			e_.stopPropagation();
			var cpath_ = $(this).attr('data-child-path');
			self_._overlay_layer.find('.le-select-overlay .le-child-dropdown').hide();
