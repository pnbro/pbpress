jQuery(function($){
	...
	**/
		...
		if(this._children_frame_el.length > 0){
			this._children_frame_el.sortable({
				...
				onMove : $.proxy(function(event_){
					...
					if(window.pbpage_builder_element_map[child_key_]['only_in_parent']){
						return $(event_.from).is(event_.to);
					}

					var children_exp_ = [];
					var parent_exp_ = window.pbpage_builder_element_map[child_key_]['parent'] ? window.pbpage_builder_element_map[child_key_]['parent'] : ["*"];
					
					if(parent_item_.length > 0){
						parent_item_ = parent_item_.pb_page_builder_element();

						parent_key_ = parent_item_._key;
						children_exp_ = window.pbpage_builder_element_map[parent_key_]['children']; 
					}else{
	...
	pb_page_builder_element.prototype.duplicate = function(){
		...
		var element_class_ = window.pbpage_builder_element_map[this._key]['edit_element_class'] ? window.pbpage_builder_element_map[this._key]['edit_element_class'] : "pb_page_builder_element";
		...
		element_instance_._element_data['id'] = "";
		element_instance_._element_data['classes'] = "";
		element_instance_._element_data['unique_class_name'] = "";
	...
	pb_page_builder_element.prototype._update_preview = function(){
		...
		var preview_fields_ = window.pbpage_builder_element_map[this._key]['preview_fields'];
		if(preview_fields_ && preview_fields_.length > 0){
			...
			$.each(preview_fields_, function(){
				...
				var preview_display_ = this['display'] ? this['display'] : "inline";
				...
				if(preview_value_){
					...
					switch(preview_type_){
						case "image" : 
							preview_value_ = "<img src='"+PBVAR['home_url']+'uploads/'+preview_value_+"'>";
							break;
						case "select" :
					...
					preview_html_ += "<div class='preview-item "+(preview_display_)+"-item'>"+preview_value_+"</div>";
				}
			});
