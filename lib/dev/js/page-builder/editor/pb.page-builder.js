jQuery(function($){
	...
	**/
	...
	var pb_page_builder_element = window.pb_page_builder_element = function(target_, page_builder_, key_, defaults_){
		...
		var element_html_ = '<div class="wrap"> \
			<div class="col-action"> \
				...
				<div class="action-frame"> \
					...
					<a href="" data-save-layout-btn class="action-btn save-layout-btn"> \
			...
			if(is_loadable_){
				element_html_ += '<div class="children-frame" data-children-frame> \
						...
						<a data-add-element-btn="prepend" class="add-element-btn prepend" href=""><i class="material-icons icon">add</i> '+__('요소추가')+'</a> \
						<a data-add-element-btn="append" class="add-element-btn append" href=""><i class="material-icons icon">add</i> '+__('요소추가')+'</a> \
				</div>';
			}
			element_html_ += '</div> \
		</div>';
		...
		this._save_layout_btn = this._target.find("[data-save-layout-btn]");
		...
		this._add_element_btns.click($.proxy(function(event_){
			...
			window._pbpagebuilder_element_picker_modal_module.pick({
				...
				if(element_id_ === '__saved_layout__'){
					this['module']._page_builder.insert_saved_layout_json(
						element_data_.layout_json, this['module']);
					return;
				}
		...
		this._save_layout_btn.click($.proxy(function(){
			this.save_as_layout();
			return false;
		},this));
	...
	pb_page_builder_element.prototype.save_as_layout = function(){
		var self_ = this;
		var element_data_ = this.to_json();

		PB.modal({
			title : __("저장서식으로 등록"),
			content : '<p>'+__("이 요소를 저장서식으로 등록합니다.")+'</p>'
				+ '<input type="text" class="form-control" name="layout_title" placeholder="'+__("서식 이름 입력")+'" autofocus>',
			button1 : __("등록"),
			button2 : __("취소"),
			callback_input : true
		}, function(confirmed_, inputs_){
			if(!confirmed_ || !inputs_['layout_title']) return;

			PB.post("admin-page-builder-save-layout", {
				title : inputs_['layout_title'],
				layout_json : JSON.stringify({
					version : "4.0.0",
					elementcontent : [element_data_]
				})
			}, function(r_, j_){
				if(!r_ || !j_.success){ PB.alert_error(j_); return; }
				PB.alert(__("저장서식으로 등록되었습니다."));
			}, true);
		});
	};
			...
			$.each(preview_fields_, function(){
				...
				if(preview_value_){
					...
					switch(preview_type_){
						case "image" :
							try {
								if(typeof preview_value_ === "string"){
									preview_value_ = JSON.parse(preview_value_);
								}
								preview_value_ = "<div class='preview-image'><div class='image' style='background-image:url(\""+PB.filebase_url(preview_value_[0]['thumbnail'])+"\")'></div></div>";
							}catch(e_){
								preview_value_ = "<div class='preview-image'><div class='image'></div></div>";
							}
	...
	**/
	var VIEW_KEY = "pb_pagebuilder_element_view";
	...
	var pb_page_builder_element_picker_modal = function(target_, options_){
		...
		// 탭 시스템
		this._tab_bar = this._target.find("[data-picker-tab-bar]");
		this._tab_content_elements = this._target.find("[data-picker-tab-content='elements']");
		this._tab_content_saved = this._target.find("[data-picker-tab-content='saved-layouts']");
		this._saved_layouts_list_el = this._target.find("[data-saved-layouts-list]");
		this._saved_layouts_loading_el = this._target.find("[data-saved-layouts-loading]");

		this._tab_bar.find("[data-picker-tab]").click($.proxy(function(e_){
			this._switch_tab($(e_.currentTarget).attr("data-picker-tab"));
			return false;
		}, this));

		// 그리드/리스트 보기 토글
		this._view_toggle = this._target.find("[data-element-view-toggle]");

		var saved_view_ = "grid";
		try { saved_view_ = window.localStorage.getItem(VIEW_KEY) || "grid"; } catch(e){}
		this._set_view(saved_view_);

		this._view_toggle.find("[data-view]").click($.proxy(function(e_){
			this._set_view($(e_.currentTarget).attr("data-view"));
			return false;
		}, this));
	}
	...
	pb_page_builder_element_picker_modal.prototype.load = function(){
		...
		PB.post("page-builder-load-element", {
			...
			$.each(this._last_element_list, function(key_, element_data_){
				...
				element_item_.find(".name").attr("title", element_data_['name'] || "");
				element_item_.find(".desc").attr("title", element_data_['desc'] || "");
	...
	pb_page_builder_element_picker_modal.prototype._set_view = function(view_){
		if(view_ !== "list") view_ = "grid";
		this._target.toggleClass("element-view-grid", view_ === "grid");
		this._target.toggleClass("element-view-list", view_ === "list");
		this._view_toggle.find("[data-view]").removeClass("active");
		this._view_toggle.find("[data-view='" + view_ + "']").addClass("active");
		try { window.localStorage.setItem(VIEW_KEY, view_); } catch(e){}
	};

	pb_page_builder_element_picker_modal.prototype._switch_tab = function(tab_){
		this._tab_bar.find("[data-picker-tab]").removeClass("active");
		this._tab_bar.find("[data-picker-tab='" + tab_ + "']").addClass("active");
		this._tab_content_elements.toggle(tab_ === "elements");
		this._tab_content_saved.toggle(tab_ === "saved-layouts");
		if(tab_ === "saved-layouts") this._load_saved_layouts();
	};

	pb_page_builder_element_picker_modal.prototype._load_saved_layouts = function(){
		var self_ = this;
		this._saved_layouts_list_el.empty();
		this._saved_layouts_loading_el.show();

		PB.post("admin-page-builder-load-saved-layouts", {}, function(r_, j_){
			self_._saved_layouts_loading_el.hide();

			if(!r_ || !j_.success){
				self_._saved_layouts_list_el.html(
					'<div class="empty-text">' + __('저장서식을 불러올 수 없습니다.') + '</div>'
				);
				return;
			}
			...
			if(!list_ || list_.length <= 0){
				self_._saved_layouts_list_el.html(
					'<div class="empty-text">' + __('등록된 저장서식이 없습니다.') + '</div>'
				);
				return;
			}

			$.each(list_, function(idx_, item_){
				var item_el_ = $(
					'<div class="element-item saved-layout-item">' +
						'<a href="" class="wrap">' +
							'<div class="col-icon"><div class="icon-wrap">' +
								'<i class="material-icons" style="font-size:40px;line-height:50px;color:#888;">folder_special</i>' +
							'</div></div>' +
							'<div class="col-name">' +
								'<div class="name"></div>' +
								'<div class="desc"></div>' +
				...
				item_el_.find(".name").text(item_.title).attr("title", item_.title || "");
				item_el_.find(".desc").text(item_.category || __('저장서식')).attr("title", item_.category || __('저장서식'));

				item_el_.click(function(){
					PB.post("admin-page-builder-get-layout-json", { id : item_.id }, function(r2_, j2_){
						if(!r2_ || !j2_.success){ PB.alert_error(j2_); return; }

						// 부모 존속성 체크
						var parent_key_ = self_._element_search_form.find("[name='parent']").val() || "document";
						var layout_data_ = (typeof j2_.layout_json === 'string') ? JSON.parse(j2_.layout_json) : j2_.layout_json;

						if(layout_data_ && layout_data_.elementcontent){
							for(var ci_ = 0; ci_ < layout_data_.elementcontent.length; ci_++){
								var root_el_name_ = layout_data_.elementcontent[ci_].name;
								var root_el_map_ = pb_page_builder_element.element_map(root_el_name_);
								if(!root_el_map_) continue;

								// 자식 요소의 parent 제약 체크
								var parent_exp_ = root_el_map_['parent'] ? root_el_map_['parent'] : ["*"];
								var can_in_parent_ = (parent_exp_.indexOf(parent_key_) >= 0 || parent_exp_.indexOf("*") >= 0)
									&& (parent_exp_.indexOf("!" + parent_key_) < 0);

								if(!can_in_parent_){
									PB.alert(__("이 저장서식은 현재 위치에 삽입할 수 없습니다."));
									return;
								}

								// 부모 요소의 children 제약 체크
								if(parent_key_ !== "document"){
									var parent_el_map_ = pb_page_builder_element.element_map(parent_key_);
									if(parent_el_map_ && parent_el_map_['children']){
										var children_exp_ = parent_el_map_['children'];
										var can_child_ = (children_exp_.indexOf(root_el_name_) >= 0 || children_exp_.indexOf("*") >= 0)
											&& (children_exp_.indexOf("!" + root_el_name_) < 0);
										if(!can_child_){
											PB.alert(__("이 저장서식은 현재 위치에 삽입할 수 없습니다."));
											return;
										}
						...
						self_._callback.apply(self_, ['__saved_layout__', {
							layout_json : j2_.layout_json
						}]);
						self_._target.modal("hide");
					});
					return false;
				});

				self_._saved_layouts_list_el.append(item_el_);
			});
		});
	};
	...
	**/
	...
	pb_page_builder.prototype.pick_element = function(options_, add_method_){
		...
		window._pbpagebuilder_element_picker_modal_module.pick({
			...
			if(element_id_ === '__saved_layout__'){
				this['module'].insert_saved_layout_json(element_data_.layout_json);
				return;
			}
			this['add_method'].apply(this['module'], [element_id_]);
		}, {
	...
	pb_page_builder.prototype.apply_xml = function(xml_string_){
		...
		var element_nodes_ = pb_page_builder._child_node_by_name(pagecontent_node_, "element", true);

		for(var child_index_ = 0; child_index_<element_nodes_.length; ++child_index_){
			var child_node_ = element_nodes_[child_index_];
			var element_id_ = child_node_.getAttribute("name");
			...
			try {
				...
				console.warn("[PageBuilder] Element '"+element_id_+"' load failed, skipping:", e_.message);
			}
		}
