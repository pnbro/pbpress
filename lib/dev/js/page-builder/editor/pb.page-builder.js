jQuery(function($){
	...
	**/
	...
	var pb_page_builder_page_settings_modal = function(target_, options_){
		...
		this._target.on("shown.bs.modal", function(){
		...
		this._modal_form.submit_handler(function(){
	...
	**/
	...
	var pb_page_builder_element = window.pb_page_builder_element = function(target_, page_builder_, key_, defaults_){
		...
		this._target.on("mouseenter", function(){
		...
		this._add_element_btns.click(function(event_){
			...
			},function(element_id_, element_data_){
		...
		this._collapse_toggle_btn.click(function(event_){
			this._children_frame_el.toggleClass("closed");
			return false;
		}.bind(this));

		this._edit_btn.click(function(){
			this.edit();
			return false;
		}.bind(this));

		this._duplicate_btn.click(function(){
			this.duplicate();
			return false;
		}.bind(this));

		this._delete_btn.click(function(){
			PB.confirm({
				title : __("삭제확인"),
		...
		if(this._children_frame_el.length > 0){
			this._children_frame_el.sortable({
				...
				animation : 100,
				onAdd : function(){this._check_children()}.bind(this),
				onRemove : function(){this._check_children()}.bind(this),
	...
	pb_page_builder_element.prototype.edit = function(){
		window._pbpagebuilder_element_edit_modal_module.edit(this._key, this._element_data, this.content(), function(results_, content_){
		...
		if(preview_fields_ && preview_fields_.length > 0){
			...
			$.each(preview_fields_, function(){
				...
				if(preview_value_){
					...
					preview_promise_.done(function(preview_html_){
	...
	**/
	var pb_page_builder_element_picker_modal = function(target_, options_){
		...
		this._element_search_form.submit_handler(function(){
			this.load();
		}.bind(this));
	}
	...
	pb_page_builder_element_picker_modal.prototype.load = function(){
		...
		PB.post("page-builder-load-element", {
			...
			$.each(this._last_element_list, function(key_, element_data_){
				...
				element_item_.click(function(event_){
	...
	**/
	var pb_page_builder_element_edit_modal = function(target_, options_){
		...
		this._edit_form.submit_handler(function(){
	...
	**/
	window.pb_page_builder = window.pb_page_builder = function(target_, options_){
		...
		this._add_element_btn.click(function(){
			this.pick_element({
				parent : "document"
			});
			return false;
		...
		this._target.find("[data-add-element-btn]").click(function(event_){
	...
	pb_page_builder.prototype._check_children = function(){
		...
		if(empty_){
			...
			this._page_element_content_list_el.find("[data-element-add-element-btn]").click(function(){
