...
function _pb_resetpass(){
	...
	PB.post_url(PB.append_url(PBVAR['home_url'], "admin/_ajax_do_resetpass.php"), {
		request_data : request_data_
	}, function(result_, response_json_){
		if(!result_ || response_json_.success !== true){
			PB.alert({
				title : response_json_.error_title || __("에러발생"),
				content : response_json_.error_message || __("비밀번호 재설정 중, 에러가 발생했습니다."), 
			});
			return;
		}
		...
		PB.alert({
			title : __("재설정 완료"),
			content : __("비밀번호를 재설정 하였습니다.<br/>로그인페이지로 이동합니다.")
		}, function(){
