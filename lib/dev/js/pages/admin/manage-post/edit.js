	...
	PB.post("edit-post", {
		...
		if(!result_ || response_json_.success !== true){
			PB.alert({
				...
				content : response_json_.error_message || __("글수정 중, 에러가 발생했습니다."),
			});
			return;
		}
...
function pb_post_add_new_category(){
	...
	PB.post("pb-admin-post-category-insert", {
		...
		if(!result_ || response_json_.success !== true){
			PB.alert(response_json_.error_title || __("에러발생"), response_json_.error_message || __("카테고리 등록 중, 에러가 발생했습니다."));
			return;
		}
