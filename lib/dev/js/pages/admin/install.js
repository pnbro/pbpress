...
function pb_install(){
	PB.confirm({
		title : "설치확인",
		content : "해당정보로 PBPress 설치를 진행하시겠습니까?",
		button1 : "설치하기"
	}, function(c_){
		if(!c_) return c_;
		_pb_install();
	...
	PB.post_url("_ajax_do_install.php", {
		...
		if(!result_ || response_json_.success !== true){
			PB.alert({
				...
				content : response_json_.error_message || "설치 중 에러가 발생했습니다.", 
			});
			return;
		}
		...
		PB.alert({
			title : "축하합니다!",
			content : "PBPress 설치가 완료되었습니다.<br/>관리자페이지로 이동합니다.",
