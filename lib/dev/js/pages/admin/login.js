	...
	PB.post_url(PB.append_url(PBVAR['home_url'], "admin/_ajax_do_login.php"), {
		...
		if(!result_ || response_json_.success !== true){
			PB.alert({
				...
				content : response_json_.error_message || __("로그인 중, 에러가 발생했습니다."), 
			});
			return;
		}
...
function _pb_do_findpass(){
		...
		PB.post_url(PB.append_url(PBVAR['home_url'], "admin/_ajax_do_findpass.php"), {
			...
			if(!result_ || response_json_.success !== true){
				PB.alert({
					title : response_json_.error_title || __("에러발생"),
					content : response_json_.error_message || __("비밀번호 재설정 메일 발송 중, 에러가 발생했습니다."),
				});
				return;
			}
			...
			PB.alert({
				title : __("메일발송완료"),
				content : __("가입하신 이메일로 비밀번호 재설정 링크를 발송하였습니다.<br/>메일을 확인하여 주세요.")
			});
