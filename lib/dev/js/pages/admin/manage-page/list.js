jQuery(document).ready(function(){
	...
	$("[data-page-status]").change(function(){
		...
		pb_manage_page_update_status(page_id_, target_el_.val(), function(){
	...
	PB.post("change-page-status",{
		...
		if(!result_ || response_json_.success !== true){
			PB.alert({
				...
				content : response_json_.error_message || __("페이지상태 수정 중, 에러가 발생했습니다."),
			});
			return;
		}
		...
		page_id : page_id_,
		status : status_.bind(callback : callback_
	}));
}
...
function pb_manage_page_register_front_page(page_id_){
	PB.confirm({
		title : __("홈화면 지정확인"),
		content : __("해당 페이지를 홈화면으로 지정하시겠습니까?"),
		button1 : __("지정하기")
	}, function(c_){
		if(!c_) return;
		PB.post("register-front-page",{
			...
			if(!result_ || response_json_.success !== true){
				PB.alert({
					title : response_json_.error_title || __("에러발생"),
					content : response_json_.error_message || __("수정 중, 에러가 발생했습니다."),
				});
				return;
			}
