jQuery(function ($) {
	...
	**/
	var pb_menu_item_edit_modal = function (target_, options_) {
		...
		this._common_form_el.submit_handler(function () {
			this._done();
		}.bind(this));
		this._meta_form_el.submit_handler(function () {
			this._done();
		}.bind(this));
	...
	pb_menu_item_edit_modal.prototype._done = function () {
		this._common_form_el.validator().validate(function (result1_) {
			if (!result1_) return;

			this._meta_form_el.validator().validate(function (result2_) {
				if (!result2_) return;
	...
	var pb_menu_editor = function (target_) {
		...
		this._menu_edit_form.find("[name='menu_title']").change(function (event_) {
		...
		this._menu_selector.change(function (event_) {
		...
		this._menu_edit_form.submit_handler(function () {
			this.do_update();
		}.bind(this));
		...
		this._menu_target_tab_content.find("form").each(function () {
			...
			target_form_el_.submit_handler(function () {
				...
				if (current_menu_id_ === null) {
					PB.alert({
						title: __("메뉴확인"),
						content: __("항목을 추가할 메뉴를 먼저 선택하세요"),
					});
					return false;
				}
		...
		this._add_menu_item_btn.click(function () {
			...
			target_form_el_.validator().validate(function (results_) {
				if (!results_) return;
				this.submit();
			}.bind(target_form_el_));
			return false;
		}.bind(this));
		...
		this._duplicate_btn.toggle(false);

		this._duplicate_btn.click(function () {
			PB.confirm({
				title: __("복제확인"),
				content: __("해당 메뉴를 복제하시겠습니까?"),
				button1: __("복제하기")
			}, function (c_) {
				if (!c_) return;
				this.do_duplicate();
	...
	pb_menu_editor.prototype.add_item = function (data_, meta_data_, add_to_) {
		...
		menu_item_.find("[data-edit-btn]").click(function (event_) {
			...
			window.pb_menu_item_edit_modal.edit(item_data_, item_meta_data_, function (t_item_data_, t_item_meta_data_) {
			...
			}.bind(t_menu_item_el_));

			return false;
		}.bind(menu_item_));

		menu_item_.find("[data-delete-btn]").click(function (event_) {

			PB.confirm({
				title: __("삭제확인"),
				content: __("해당 메뉴항목을 삭제하시겠습니까?"),
				button1: __("삭제하기"),
	...
	pb_menu_editor.prototype.remove_item = function (menu_item_) {
		...
		menu_item_.children(".wrap").animate({ opacity: 0 }, 300, function () {
	...
	pb_menu_editor.prototype.load_menu = function (menu_id_) {
		...
		PB.post("menu-editor-load-menu", {
			...
			if (!result_ || response_json_.success !== true) {
				PB.alert({
					title: response_json_.error_title || __("에러발생"),
					content: response_json_.error_message || __("메뉴정보를 불러오는 중, 에러가 발생했습니다"),
				});
				return;
			}
			...
			this._duplicate_btn.toggle(true);
	...
	pb_menu_editor.prototype.do_delete = function () {
		...
		PB.post("menu-editor-do-delete", {
			...
			if (!result_ || response_json_.success !== true) {
				PB.alert({
					title: response_json_.error_title || __("에러발생"),
					content: response_json_.error_message || __("메뉴 삭제 중, 에러가 발생했습니다"),
				});
				return;
			}
	...
	pb_menu_editor.prototype.do_duplicate = function () {
		var menu_id_ = this.current_menu_id();
		if (!menu_id_ || menu_id_ < 0) return;

		PB.post("menu-editor-do-duplicate", {
			menu_id: menu_id_,
		}, function (result_, response_json_) {
			if (!result_ || response_json_.success !== true) {
				PB.alert({
					title: response_json_.error_title || __("에러발생"),
					content: response_json_.error_message || __("메뉴 복제 중, 에러가 발생했습니다"),
				});
				return;
			}
	...
	pb_menu_editor.prototype.do_update = function () {
		...
		PB.post("menu-editor-do-update", {
			...
			if (!result_ || response_json_.success !== true) {
				PB.alert({
					title: response_json_.error_title || __("에러발생"),
					content: response_json_.error_message || __("메뉴 저장 중, 에러가 발생했습니다"),
				});
				return;
			}
