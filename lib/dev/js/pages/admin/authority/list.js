...
function pb_manage_authority_task_update(){
	...
	PB.confirm({
		title : __("권한부여확인"),
		content : __("해당 작업에 대한 권한을 부여하시겠습니까?"),
		button1 : __("부여하기")
	}, function(c_){
		if(!c_) return;
	...
	PB.post("pb-admin-authority-task-update",{
		...
		if(!result_ || response_json_.success !== true){
			PB.alert({
				...
				content : response_json_.error_message || __("권한부여 중, 에러가 발생했습니다."),
			});
			return;
		}
