(function($){
	...
	PB = $.extend(PB, {
		modal : function(options_, callback_){
			...
			if($.type(arguments[0]) === "string"){
				options_ = {};
				options_['title'] = arguments[0];
				options_['content'] = arguments[1];
				options_['button1'] = arguments[2];
				options_['button2'] = arguments[3];
				if(arguments[4]){
					callback_ = arguments[4];
				}
			}else{
				options_ = arguments[0];
				callback_ = arguments[1];
			}
	...
	PB._post = PB.post;
	PB.post = function(action_, data_, callback_, block_, options_){
		if(block_ !== undefined && block_ === true){
			this.indicator(true, function(){
				PB._post.apply(PB, [action_, data_, function(result_, response_text_, cause_){
					PB.indicator(false);
					callback_(result_, response_text_, cause_);
				}, options_]);
			});
		}else{
			return PB._post.apply(PB,[action_, data_, callback_, options_]);
		}
	};
