(function($){
	...
	var _pb_image_uploader_btn = function(target_, options_){
		...
		this._options = $.extend(true, {
			'upload_path' : null,
			'maxlength' : window.pb_image_uploader_btn.maxlength,
			'modal_label' : window.pb_image_uploader_btn.modal_label,
			'use_thumbnail' : window.pb_image_uploader_btn.use_thumbnail,
			'accept' : window.pb_image_uploader_btn.default.accept,
			'callback' : $.noop,
		}, options_);
			...
			'<div class="modal-dialog">' +
				'<div class="modal-content">' +
					'<div class="modal-body">' +
						'<input id="pb-image-uploader-modal-input-'+this._modal_uid+'" type="file" name="files[]" accept="'+this._options['accept']+'" multiple>' +
					'</div>' +
	...
	window.pb_multiple_image_uploader = {
		default : {
			'max_length' : 10,
			'modal_label' : __('이미지업로드'),
			'accept' : "image/*",
	...
	var _pb_multiple_image_uploader = function(target_, options_){
		...
		this._options = $.extend(true, {
			'upload_path' : null,
			'maxlength' : window.pb_multiple_image_uploader.default['maxlength'],
			'modal_label' : window.pb_multiple_image_uploader.default['modal_label'],
			'accept' : window.pb_multiple_image_uploader.default.accept,
			'change' : $.noop,
			'max_width' : null,
			'input_to_path' : false,
		}, options_);
		...
		var modal_html_ = '<div class="pb-imageupload-dropzone-modal modal" id="pb-multiple-image-uploader-modal-'+this._modal_uid+'">' +
			'<div class="modal-dialog">' +
				'<div class="modal-content">' +
					'<div class="modal-body">' +
						'<input id="pb-multiple-image-uploader-modal-input-'+this._modal_uid+'" type="file" name="files[]" accept="'+this._options['accept']+'" multiple>' +
					'</div>' +
					'<div class="modal-footer">' +
						'<button type="button" class="btn btn-default" data-dismiss="modal">'+__('취소')+'</button>' +
					'</div>' +
