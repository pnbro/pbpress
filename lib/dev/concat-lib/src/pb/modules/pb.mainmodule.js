(function($){
	...
	var PB = window.PB = {
		...
		// =====================================================================
		// chunk_text: 범용 텍스트 청크 업로드
		// =====================================================================
		__chunk_text_maps : {},

		chunk_text : function(text_, options_, callback_){
			callback_ = callback_ !== undefined ? callback_ : $.noop;

			options_ = $.extend({
				action     : null,
				chunk_size : (typeof PBVAR !== 'undefined' && PBVAR['file_chunksize']) ? PBVAR['file_chunksize'] : (1024 * 1024),
				params     : {},
				start      : $.noop,
				progress   : $.noop,
				done       : $.noop,
			...
			if(!options_.action){
				console.error("PB.chunk_text: 'action' is required");
				callback_(false, null);
				return;
			}

			var encoder_      = new TextEncoder();
			var bytes_        = encoder_.encode(text_);
			var byte_len_     = bytes_.length;
			var chunk_size_   = options_.chunk_size;
			var chunk_length_ = Math.ceil(byte_len_ / chunk_size_);
			...
			var upload_key_ = PB.random_string(10);
			var chunk_map_  = {
				upload_key       : upload_key_,
				chunk_current    : 0,
				chunk_length     : chunk_length_,
				chunk_size       : chunk_size_,
				text_byte_length : byte_len_,
				upload_xhr       : null
			};

			PB.__chunk_text_maps[upload_key_] = chunk_map_;
			options_.start.apply(window, [chunk_map_]);

			var decoder_ = new TextDecoder();

			function send_next_(){
				var map_ = PB.__chunk_text_maps[upload_key_];
				if(!map_) return;

				var start_byte_ = map_.chunk_current * map_.chunk_size;
				var end_byte_   = Math.min(start_byte_ + map_.chunk_size, byte_len_);
				var chunk_text_ = decoder_.decode(bytes_.slice(start_byte_, end_byte_));

				var post_data_ = $.extend({}, options_.params, {
					_chunk_upload_key : upload_key_,
					_chunk_current    : map_.chunk_current,
					_chunk_length     : map_.chunk_length,
					_chunk_data       : chunk_text_
				});

				map_.upload_xhr = PB.post(options_.action, post_data_, function(result_, json_, raw_){
					if(!result_ || (json_ && !json_.success)){
						options_.fail.apply(window, [json_, raw_]);
						callback_(false, json_);
						delete PB.__chunk_text_maps[upload_key_];
						return;
					}

					map_.chunk_current++;
					var percent_ = (map_.chunk_current / map_.chunk_length) * 100;
					options_.progress.apply(window, [percent_, map_]);

					if(map_.chunk_current < map_.chunk_length){
						PB.__chunk_text_maps[upload_key_] = map_;
						send_next_();
					}else{
						options_.done.apply(window, [json_]);
						callback_(true, json_);
						delete PB.__chunk_text_maps[upload_key_];
					}
				});
			}
		...
		chunk_text_abort : function(upload_key_){
			var map_ = PB.__chunk_text_maps[upload_key_];
			if(map_ && map_.upload_xhr){
				map_.upload_xhr.abort();
			}
			delete PB.__chunk_text_maps[upload_key_];
		}
	}
