(function($){
	...
	var pb_fileupload_btn = (function(target_, options_){
		...
		this._options = $.extend({
			...
			label : __('파일선택'),
			label_dropzone : __('파일을 드래그하여 업로드할 수 있습니다.'),
		...
		var done_override_ = $.proxy(function(event_, data_){
		...
		if(this._options['done_override']){
			done_override_ = $.proxy(function(event_, data_){
				this._options['done_override'].apply(this, [event_, data_]);
			}, this);	
		}
