(function ($) {
	PB.file = $.extend(PB.file, {
		...
		supports_chunk_upload: function () {
			return PBVAR['file_chunk_upload_supported'] !== false;
		},
		compatible_upload_url: function () {
			return this.supports_chunk_upload() ? this.chunk_upload_url() : this.upload_url();
		},
		__upload_xhr: null,
		__upload_chunk_maps: {},
		__upload_chunk(key_, files_, options_, callback_) {
			...
			if (!PB.file.__upload_chunk_maps[key_]) {
				...
				for (var file_index_ = 0; file_index_ < files_.length; ++file_index_) {
					var target_file_ = files_[file_index_];
			...
			PB.file.__upload_chunk_maps[key_]['upload_xhr'] = $.ajax({
				...
				success: $.proxy(function (response_) {
					...
					try {
						response_json_ = typeof response_ === 'string' ? JSON.parse(response_) : response_;
					} catch (ex_) {
						var failed_results_ = this['chunk_map_data']['results'];
						delete PB.file.__upload_chunk_maps[this['key']];
						this['options']['fail'].apply(window, [response_json_, response_]);
						this['callback'].apply(window, [false, failed_results_, response_json_, response_]);
						return;
					}
					if (!response_json_ || response_json_.success !== true || !response_json_.results) {
						var failed_results_ = this['chunk_map_data']['results'];
						delete PB.file.__upload_chunk_maps[this['key']];
						this['options']['fail'].apply(window, [response_json_, response_]);
						this['callback'].apply(window, [false, failed_results_, response_json_, response_]);
					...
					if (chunk_map_data_['total_chunk_current'] < chunk_map_data_['total_chunk_length']) {
						...
						var completed_results_ = chunk_map_data_['results'];
						delete PB.file.__upload_chunk_maps[this['key']];
						this['options']['done'].apply(window, [response_json_, completed_results_, response_]);
						this['callback'].apply(window, [true, completed_results_, response_json_, response_]);
				...
				error: $.proxy(function (response_) {
					var response_json_ = null;
					try {
						response_json_ = response_.responseJSON || JSON.parse(response_.responseText);
					} catch (ex_) { }
		...
		upload: function (files_, options_, callback_) {
			...
			this._current_xhr = $.ajax({
				...
				success: $.proxy(function (response_) {
					...
					this['callback'].apply(window, [true, response_json_, response_]);
				}, {
					options: options_,
					callback: callback_,
				}),
	...
	var pb_fileupload_btn = (function (target_, options_) {
		...
		this._options = $.extend({
			...
			chunk_upload: PB.file.supports_chunk_upload(),
			accept: null,
			limit: null,
	...
	pb_fileupload_btn.prototype.options = function (options_) {
