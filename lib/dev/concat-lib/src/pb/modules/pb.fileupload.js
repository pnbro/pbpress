(function($){
		...
		__upload_chunk(key_, files_, options_, callback_){
			...
			PB.file.__upload_chunk_maps[key_]['upload_xhr'] = $.ajax({
				...
				xhr: function(){
					...
					xhr_.upload.addEventListener("progress", function(event_){
	...
	pb_fileupload_btn.prototype.__upload = (function(){
		...
		if(this._options['chunk_upload']){
			...
			PB.file.chunk_upload(this._target[0].files, options_, function(result_, upload_results_, response_json_, response_text_){
			if(!result_){
			...
			PB.file.upload(this._target[0].files, this._options, function(result_, response_json_){
				if(!result_){
					this.toggle_loading(false);
					return;
				}
