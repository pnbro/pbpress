jQuery(function () {
	...
	var _pb_edit_form_modal = (function (target_, options_) {
		...
		this._options = $.extend({
			...
			'btn_insert_text': __("추가하기"),
			'btn_update_text': __("수정하기"),
			'btn_delete_text': __("삭제"),
			'btn_cancel_text': __("취소"),

			'title_text_for_insert': __("추가하기"),
			'title_text_for_update': __("수정하기"),
		...
		this._btn_submit = $("<button type='submit' class='btn btn-primary'>" + this._options['btn_insert_text'] + "</button>");
		this._btn_delete = $("<button type='button' class='btn btn-dark'>" + this._options['btn_delete_text'] + "</button>");
		this._btn_cancel = $("<button type='button' class='btn btn-default' data-bs-dismiss='modal'>" + this._options['btn_cancel_text'] + "</button>");
		...
		this._target.validator();
		this._target.submit_handler(function () {
		...
		this._modal.on("shown.bs.modal", function () {
			this._target.validator().refresh();
		}.bind(this));

		this._btn_submit.click(function () {
			this._target.submit();
			return false;
		}.bind(this));

		this._btn_delete.click(function () {
			this.do_delete();
		}.bind(this));
	...
	_pb_edit_form_modal.prototype.open_for_update = (function (key_) {
		...
		this._btn_delete.text(this._options['btn_delete_text']);
		...
		PB.post(this._options['load_action'], {
			...
			if (!result_ || response_json_.success !== true) {
				PB.alert({
					...
					content: response_json_.error_message || __("자료를 불러오는 중, 에러가 발생했습니다."),
				});
				return;
			}
	...
	_pb_edit_form_modal.prototype.do_insert = (function () {
		...
		this._options['before_insert'].apply(this, [target_data_, function (result_) {
			if (!result_) return;
			...
			PB.post(this._options['insert_action'], {
				...
				if (!result_ || response_json_.success !== true) {
					PB.alert({
						title: response_json_.error_title || __("에러발생"),
						content: response_json_.error_message || __("자료 추가 중, 에러가 발생했습니다."),
					});
					return;
				}

				this._modal.modal("hide");
				this._options['after_inserted'].apply(this, [this._target, response_json_]);
	...
	_pb_edit_form_modal.prototype.do_update = (function () {
		...
		this._options['before_update'].apply(this, [target_data_, function (result_) {
			if (!result_) return;
			...
			PB.post(this._options['update_action'], {
				...
				if (!result_ || response_json_.success !== true) {
					PB.alert({
						title: response_json_.error_title || __("에러발생"),
						content: response_json_.error_message || __("자료 수정 중, 에러가 발생했습니다."),
					});
					return;
				}

				this._modal.modal("hide");
				this._options['after_updated'].apply(this, [this._target, response_json_]);
	...
	_pb_edit_form_modal.prototype.do_delete = (function (key_) {
		...
		this._options['before_delete'].apply(this, [this._target.serialize_object(), function (result_) {
			if (!result_) return;
			...
			PB.post(this._options['delete_action'], {
				...
				if (!result_ || response_json_.success !== true) {
					PB.alert({
						title: response_json_.error_title || __("에러발생"),
						content: response_json_.error_message || __("자료 삭제 중, 에러가 발생했습니다."),
					});
					return;
				}
				...
				this._options['after_deleted'].apply(this, [this._target, response_json_]);
