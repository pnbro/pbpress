jQuery(function($){
	var _pb_easy_splitted_view = (function(target_, options_){
		...
		var detail_table_id_ = this._target.attr("data-detail-table-id");
		

		this._exists_detail_table = !!(detail_table_id_);

		if(this._exists_detail_table){
		...
		this._master_table.on("pbeasytableloaded", function(event_, row_data_, row_el_){
			this.current_master_id(null);
			
			row_el_.find("a[data-master-id]").click(function(event_){
		...
		this._col_detail_el.children(".mobile-close-btn").click(function(){
		...
		//initialize master table
		if(this._exists_detail_table){
			this._detail_table.on("pbeasytableloaded", function(event_, row_data_, row_el_){
	...
	_pb_easy_splitted_view.prototype.current_master_id = (function(master_id_){
		if(master_id_ !== undefined){
			...
			if(this._current_master_id){
				this.load_master_info(master_id_, function(){
	...
	_pb_easy_splitted_view.prototype.load_detail_list = (function(master_id_){
		...
		if(this._exists_detail_table){
			...
			this._detail_table.pbeasytable().page_index(0);
	...
	_pb_easy_splitted_view.prototype.refresh = (function(){
		...
		this._master_table_module.load(function(){
