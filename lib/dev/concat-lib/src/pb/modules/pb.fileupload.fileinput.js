(function($){
	...
	window.pb_file_input = {
		default : {
			'upload_url' : PB.file.compatible_upload_url(),
	...
	window._pb_file_input = function(target_, options_){
		...
		var accept_ = options_['accept'] || this._target.attr("data-accept");
			accept_ = !accept_ ? window.pb_file_input.default['accept'] : accept_;

		var single_ = options_['single'] || this._target.attr("data-single");
			single_ = !!(single_ === "Y");
		
		var limit_ = options_['limit'] || this._target.attr("data-limit");
		...
		this._dropzone_module = this._dropzone_input.pb_file_dragzone({
			...
			chunk_upload : this._options['chunk_upload'],
	...
	_pb_file_input.prototype.options = function(options){
		...
		if(this._options['chunk_upload'] !== undefined){
			this._dropzone_module.options({chunk_upload : this._options['chunk_upload']});
		}
