(function($){
	...
	var _pb_file_uploader_btn = function(target_, options_){
		...
		this._options = $.extend(true, {
			'upload_path' : null,
			'maxlength' : window.pb_file_uploader_btn.default['maxlength'],
			'accept' : window.pb_file_uploader_btn.default['accept'],
			'modal_label' : window.pb_file_uploader_btn.default['modal_label'],
			'callback' : $.noop,
		}, options_);
			...
			'<div class="modal-dialog">' +
				'<div class="modal-content">' +
					'<div class="modal-body">' +
						'<input id="pb-file-uploader-modal-input-'+this._modal_uid+'" type="file" name="files[]" accept="'+this._options['accept']+'" multiple>' +
					'</div>' +
	...
	window.pb_multiple_file_uploader = {
		default : {
			'maxlength' : 10,
			'accept' : "image/*,.zip,.pdf,.hwp,.xls,.xlsx,.doc,.docx,*.ppt,*.pptx",
			'modal_label' : __('파일업로드'),
	...
	var _pb_multiple_file_uploader = function(target_, options_){
		...
		this._options = $.extend(true, {
			'upload_path' : null,
			'maxlength' : window.pb_multiple_file_uploader.default['maxlength'],
			'accept' : window.pb_multiple_file_uploader.default['accept'],
			'modal_label' : window.pb_multiple_file_uploader.default['modal_label'],
			'change' : $.noop
		}, options_);
		...
		this._add_btn = $("<a href='#' class='add-btn btn btn-sm btn-default'><i class='material-icons'>add</i> "+__("파일추가")+"</a>");
		...
		var modal_html_ = '<div class="pb-fileupload-dropzone-modal modal" id="pb-multiple-file-uploader-modal-'+this._modal_uid+'">' +
			'<div class="modal-dialog">' +
				'<div class="modal-content">' +
					'<div class="modal-body">' +
						'<input id="pb-multiple-file-uploader-modal-input-'+this._modal_uid+'" type="file" name="files[]" accept="'+this._options['accept']+'" multiple>' +
					'</div>' +
					'<div class="modal-footer">' +
