(function($){
	...
	var pb_file_dragzone = function(target_, options_){
		...
		this._inner_selected_button.click(function(){
			this._target.click();
			return false;
		}.bind(this));
		...
		this._inner_noselect_button.click(function(){
			this._target.click();
			return false;
		}.bind(this));
		...
		this._target.on("change", function(){
	...
	pb_file_dragzone.prototype.add_files = function(files_, check_valid_){
		...
		for(var file_index_=0;file_index_<file_length_;++file_index_){
			...
			if(is_image_){
				...
				file_reader_.onload = function(event_){
					this.find(".icon.image").append("<img src='"+event_.target.result+"' class='preview'>");
				}.bind(preview_file_item_);
				file_reader_.readAsDataURL(accept_file_);
			}

			preview_file_item_.find("[data-delete-button]").click(function(){
	...
	pb_file_dragzone.prototype.upload = function(callback_){
		...
		PB.file.chunk_upload(this._selected_files, {
			...
			progress : function(total_percent_, chunk_map_data_){
