	...
	pb_file_dragzone.prototype.upload = function(callback_){
		...
		var upload_options_ = {
			...
			fail : $.proxy(function(response_json_, response_){
				...
				if(!this['module']._options['chunk_upload']){
					results_ = response_json_ && $.isArray(response_json_.files) ? response_json_.files : [];
				}
		...
		if(this._options['chunk_upload']){
			PB.file.chunk_upload(this._selected_files, upload_options_);
		}else{
			PB.file.upload(this._selected_files, upload_options_);
		}
