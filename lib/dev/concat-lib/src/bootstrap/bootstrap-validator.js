(function($){
	...
	pb_validator.prototype.refresh = (function(){
		...
		this._inputs.each(function(){
			var input_el_ = $(this);
			var input_type_ = input_el_.attr("type");

			switch(input_type_){
				case "radio" :
				case "checkbox" :
					input_el_.off("click", _pb_validator_input_event);
					input_el_.on("click", _pb_validator_input_event);
					break;
				default:
					input_el_.off("change keyup dp.change dp.update", _pb_validator_input_event);
					input_el_.on("change keyup dp.change dp.update", _pb_validator_input_event);
					break;
			}
		});
		

		this._inputs = this._inputs.filter(function(){
	...
	pb_validator.input_value = (function(input_){
		...
		if(input_type_ === "radio" || input_type_ === "checkbox"){
			var form_el_ = input_.closest('form');
			return form_el_.find(":input[name='"+input_.attr("name")+"']:checked").val();
		}
	...
	pb_validator.DEFAULTS = {
		...
		validators : {
			...
			'common' : function(input_, callback_){ //common
				...
				var input_type_ = input_.attr("type");
				var is_radio_ = (input_type_ === "radio" || input_type_ === "checkbox");

				if(is_radio_){
					var t_input_ = input_.closest("form").find("[name='"+input_.attr("name")+"'][required]");
					if(t_input_.length > 0) input_ = t_input_;
				}
			...
			'function-validator' : function(input_, callback_){
				var input_value_ = pb_validator.input_value(input_);
				var function_ = input_.attr("data-function-validator");
				if(!window[function_]){
					callback_(false, pb_validator.error_message(input_, 'function-validator', 'function not found'));
				}

				window[function_].apply(input_, [input_, input_value_, $.proxy(function(result_){
					if(result_){
						this['callback'](result_, pb_validator.error_message(this['input'], 'function-validator', 'not vaild'));	
					}else{
						this['callback'](result_, pb_validator.error_message(this['input'], 'function-validator', 'not vaild'));
					}
