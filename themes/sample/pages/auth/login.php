	...
	<div class="auth-shell">
		<div class="card auth-shell__card">
			<div class="card-header text-center">
				...
				<p class="text-muted text-sm"><?=__('이메일로 로그인합니다.')?></p>
			</div>
			...
			<div class="card-body">
				<form id="sample-auth-login-form" class="auth-shell__form" data-validate novalidate>
					...
					<div class="field">
						...
						<input type="email" id="sample-auth-login-email" name="user_email" class="input" autocomplete="email" placeholder="you@example.com" data-required data-required-message="<?=__('이메일을 입력하세요.')?>" data-email data-email-message="<?=__('이메일 형식이 올바르지 않습니다.')?>">
					</div>
					...
					<button type="submit" class="btn btn-primary btn-block" data-default-label="<?=__('로그인')?>" data-loading-label="<?=__('로그인 중...')?>"><?=__('로그인')?></button>
				</form>
			</div>

			<div class="card-footer auth-shell__links">
				<a href="<?=pb_resetpass_url()?>"><?=__('비밀번호를 잊으셨나요?')?></a>
				<a href="<?=pb_signup_url()?>"><?=__('회원가입')?></a>
			</div>
		</div>
	</div>
</div>
