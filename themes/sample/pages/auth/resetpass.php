	...
	<div class="auth-shell">
		<div class="card auth-shell__card">
			...
			<div class="card-body">
				...
				<?php if($sample_auth_resetpass_view_state_['step'] === 'invalid'){ ?>
					...
					<p class="text-center mt-4">
						<a href="<?=pb_resetpass_url()?>" class="btn btn-ghost"><?=__('다시 요청하기')?></a>
					</p>

				<?php }else if($sample_auth_resetpass_view_state_['step'] === 'form'){ ?>

					<p class="text-muted text-sm mb-4"><?=__('새로운 비밀번호를 입력하세요.')?></p>
					...
					<form id="sample-auth-resetpass-change-form" class="auth-shell__form" data-validate novalidate>
						...
						<div class="field">
							<label class="label label-required" for="sample-auth-resetpass-pass"><?=__('새 비밀번호')?></label>
							<input type="password" id="sample-auth-resetpass-pass" name="user_pass" class="input" autocomplete="new-password" placeholder="<?=__('4자 이상')?>" data-required data-required-message="<?=__('비밀번호를 입력하세요.')?>" data-min="4" data-min-message="<?=__('비밀번호는 4자 이상이어야 합니다.')?>">
						</div>

						<div class="field">
							<label class="label label-required" for="sample-auth-resetpass-pass-confirm"><?=__('비밀번호 확인')?></label>
							<input type="password" id="sample-auth-resetpass-pass-confirm" name="user_pass_confirm" class="input" autocomplete="new-password" placeholder="<?=__('비밀번호 재입력')?>" data-required data-required-message="<?=__('비밀번호를 다시 입력하세요.')?>" data-match="#sample-auth-resetpass-pass" data-match-message="<?=__('비밀번호가 일치하지 않습니다.')?>">
						</div>

						<button type="submit" class="btn btn-primary btn-block" data-default-label="<?=__('비밀번호 변경')?>" data-loading-label="<?=__('처리 중...')?>"><?=__('비밀번호 변경')?></button>
					</form>

				<?php }else{ ?>

					<p class="text-muted text-sm mb-4"><?=__('가입한 이메일로 재설정 링크를 보내드립니다.')?></p>
					...
					<form id="sample-auth-resetpass-request-form" class="auth-shell__form" data-validate novalidate>
						<div class="field">
							...
							<input type="email" id="sample-auth-resetpass-email" name="user_email" class="input" autocomplete="email" placeholder="you@example.com" data-required data-required-message="<?=__('이메일을 입력하세요.')?>" data-email data-email-message="<?=__('이메일 형식이 올바르지 않습니다.')?>">
						</div>

						<button type="submit" class="btn btn-primary btn-block" data-default-label="<?=__('재설정 메일 보내기')?>" data-loading-label="<?=__('발송 중...')?>"><?=__('재설정 메일 보내기')?></button>
					</form>
			...
			<div class="card-footer auth-shell__links">
				<a href="<?=pb_login_url()?>"><?=__('로그인으로 돌아가기')?></a>
			</div>
		</div>
	</div>
</div>
